...
namespace TerminalFAtest.Models.KKTRequest
{
    ...
    public class PaymentData
    {
            ...
            )
        {
            ...
            string clientEmail = ClientEmail; if (clientEmail.Length > 64) clientEmail = clientEmail.Substring(0, 64); // обрезка 
            string clientINN = ClientINN; if (clientINN.Length > 12) clientINN = clientINN.Substring(0, 12); // обрезка 
            string cashierFIO = CashierFIO; if (cashierFIO.Length > 64) cashierFIO = cashierFIO.Substring(0, 64); // обрезка 
            string cashierINN = CashierINN; 
            if (cashierINN.Length > 12) cashierINN = cashierINN.Substring(0, 12); // обрезка 
            if (cashierINN.Length < 12) cashierINN = cashierINN.PadLeft(12, ' '); // добивка пробелами до 12 символов 
            string сlientName = ClientName; if (сlientName.Length > 255) сlientName = сlientName.Substring(0, 255); // обрезка 
            string сustomReq = CustomReq; if (сustomReq.Length > 16) сustomReq = сustomReq.Substring(0, 16); // обрезка 
            string сustomUserReqName = CustomUserReqName; if (сustomUserReqName.Length > 64) сustomUserReqName = сustomUserReqName.Substring(0, 64); // обрезка 
            string сustomUserReqValue = CustomUserReqValue; if (сustomUserReqValue.Length > 255) сustomUserReqValue = сustomUserReqValue.Substring(0, 255); // обрезка 
            ...
            //this.CustomUserReqValue = new KKTRequestProperty<string>() { TAG = 1086, USER_VALUE = CustomUserReqValue };
        }
    ...
    */
    ...
    //// список всех платежей для конкретной PaymentData:
    //public class Payments
    //{
    //    public KKTRequestProperty<>
    //}
}